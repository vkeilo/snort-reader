...
# include "config.h"		//硬件相关
#endif

#ifdef HAVE_GETTID
#define _GNU_SOURCE			//********？？？	https://blog.csdn.net/qq_31213895/article/details/88960165?biz_id=102&utm_term=_GNU_SOURCE&utm_medium=distribute.pc_search_result.none-task-blog-2~all~sobaiduweb~default-0-88960165&spm=1018.2118.3001.4187
#endif

//维基百科 C语言标准函数库 https://zh.wikipedia.org/wiki/C%E6%A8%99%E6%BA%96%E5%87%BD%E5%BC%8F%E5%BA%AB
//C语言标准库	https://www.runoob.com/cprogramming/c-standard-library-errno-h.html

#include <assert.h>			//assert只是对所给的表达式求值，就像if判断语句中一样，然后如果该值为真则正常运行，否则报错，并调用abort(),产生异常中断，exit出来
#include <sys/types.h>		//基本系统数据类型		是Unix / Linux系统的基本系统数据类型的头文件，含有size_t，time_t，pid_t等类型。
#include <sys/stat.h>		//stat函数可以返回一个结构，里面包括文件的全部属性。	https://blog.csdn.net/zhangzheng0413/article/details/6921410?utm_medium=distribute.pc_relevant.none-task-blog-OPENSEARCH-2.channel_param&depth_1-utm_source=distribute.pc_relevant.none-task-blog-OPENSEARCH-2.channel_param
#include <fcntl.h>			//主要是一些对系统的文件操作	http://blog.sina.com.cn/s/blog_67eb1f2f0100m0sa.html
#include <unistd.h>			//用于linux/unix获取进程id、等系统操作，fcntl.h也有需要用到
#include <errno.h>			//一堆错误宏的定义
#include <sys/types.h>		
#include <stdlib.h>			//类型转换、内存收放、随机数生成、终止abort、退出exit
#include <stdio.h>

#include <string.h>
#include <setjmp.h>			//微软编写的这个库可以用来进行问题处理	https://blog.csdn.net/weixin_33922670/article/details/91921779?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522160119429319725271737665%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=160119429319725271737665&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~first_rank_ecpm_v3~pc_rank_v2-1-91921779.first_rank_ecpm_v3_pc_rank_v2&utm_term=setjmp.h&spm=1018.2118.3001.4187
#include <signal.h>			//信号处理	https://blog.csdn.net/fjt19900921/article/details/8132220?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522160119446119195188327027%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=160119446119195188327027&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~sobaiduend~default-3-8132220.first_rank_ecpm_v3_pc_rank_v2&utm_term=signal.h&spm=1018.2118.3001.4187
#include <time.h>
#include <pcap.h>			//网络开发包（常用于网络安全），能够构造任意（包括非法的）网络数据包并发送，还能够截获IP数据
#ifdef HAVE_MALLOC_TRIM
#include <malloc.h>
#endif

#ifndef WIN32
#include <netdb.h>			//Unix和Linux特有的头文件，主要定义了与网络有关的结构、变量类型、宏、函数等。	https://blog.csdn.net/chenhaojin/article/details/38758013?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522160119584119725271763294%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=160119584119725271763294&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~first_rank_ecpm_v3~pc_rank_v2-1-38758013.first_rank_ecpm_v3_pc_rank_v2&utm_term=netdb.h&spm=1018.2118.3001.4187
#else
#include <Iphlpapi.h>		//windows的平台sdk中有一个很重要的iphlpapi.h头文件，它提供了一些非常实用的获取网络信息（网卡、tcp/ip等）的函数：
#endif

#ifdef HAVE_GETOPT_LONG		//unistd.h下的函数，********不太清楚
//#define _GNU_SOURCE
/* A GPL copy of getopt & getopt_long src code is now in sfutil，sfutil中现已包含getopt和getopt_long src代码的GPL副本 */
# undef HAVE_GETOPT_LONG	//取消宏定义
#endif
#include <getopt.h>			//getopt函数是命令行参数解析函数	https://blog.csdn.net/c1523456/article/details/79173776?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522160119620819725271722750%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=160119620819725271722750&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~sobaiduend~default-2-79173776.first_rank_ecpm_v3_pc_rank_v2&utm_term=getopt.h&spm=1018.2118.3001.4187

#ifdef HAVE_STRINGS_H
# include <strings.h>		//可能是string.h的扩展 https://blog.csdn.net/weixin_30569001/article/details/96901101?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522160119627719195188319336%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=160119627719195188319336&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~first_rank_ecpm_v3~pc_rank_v2-1-96901101.first_rank_ecpm_v3_pc_rank_v2&utm_term=strings.h&spm=1018.2118.3001.4187
#endif

#ifndef WIN32
# include <grp.h>
# include <pwd.h>
# include <sys/socket.h>	//socket是一个用于程序和操作系统之间，进行网络数据收发的接口，*******为什么windows不需要？
# include <netinet/in.h>	//网络数据操作（协议、地址）https://pubs.opengroup.org/onlinepubs/000095399/basedefs/netinet/in.h.html
# include <arpa/inet.h>		//主要是信息转换，将客户端信息，转换为字符串信息	https://blog.csdn.net/VCCTor/article/details/50976443?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522160119711519195246626340%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=160119711519195246626340&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~first_rank_ecpm_v3~pc_rank_v2-2-50976443.first_rank_ecpm_v3_pc_rank_v2&utm_term=arpa%2Finet.h&spm=1018.2118.3001.4187
#endif  /* !WIN32 */

#if !defined(CATCH_SEGV) && !defined(WIN32)		//*******
# include <sys/resource.h>	//用来包含sys目录下的resource.h头文件。头文件sys / resource.h为资源操作提供了定义。需要科学上网	http://opengroup.org/onlinepubs/007908775/xsh/sysresource.h.html
...
#include "active.h"
#include "snort.h"
#include "rules.h"			//规则相关
#include "treenodes.h"		//树结点
#include "plugbase.h"		//插件相关	Snort当前支持的插件包括:数据库日志输出插件，破碎数据包检测插件，断口扫描检测插件，HTTP URI插件，XML网页生成等插件	使用其IPTables,IPFilter插件可以使入侵检测主机与防火墙联动，通过FlexResp功能，Snort能够命令防火墙主动短开恶意连接
#include "snort_debug.h"	//调试
#include "util.h"
#include "parser.h"			//解析器
#include "tag.h"			//标签
#include "log.h"			//日志
#include "detect.h"			//探测
#include "mstring.h"		//用于字符串匹配	https://blog.csdn.net/pangyemeng/article/details/50924229?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522160130051519725255523661%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=160130051519725255523661&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~first_rank_ecpm_v3~pc_rank_v2-1-50924229.first_rank_ecpm_v3_pc_rank_v2&utm_term=INTEL_SOFT_CPM&spm=1018.2118.3001.4187
#include "fpcreate.h"		//****创建
#include "fpdetect.h"		//****探测
#include "sfthreshold.h"
#include "rate_filter.h"
#include "packet_time.h"	//数据包时间相关
#include "detection-plugins/sp_flowbits.h"	//检测插件
#include "preprocessors/spp_perfmonitor.h"	//预处理器/性能监视器
#include "preprocessors/perf-base.h"
#include "preprocessors/perf.h"
#include "mempool.h"		//内存池管理
#include "strlcpyu.h"		//******
#include "sflsq.h"
#include "sp_replace.h"		//******替换
#include "output-plugins/spo_log_tcpdump.h"//输出插件/命令日志
#include "event_queue.h"	//事件队列
#include "asn1.h"			//*****网络相关
#include "mpse.h"			//*****网络相关
#include "generators.h"		//发生器
#include "ppm.h"			//*****百万分之一
#include "profiler.h"		//分析器
#include "dynamic-plugins/sp_dynamic.h"			//动态插件
#include "dynamic-plugins/sf_dynamic_define.h"	//
#include "dynamic-output/plugins/output.h"		//动态输出
#include "sfutil/strvec.h"			//******
#include "detection_util.h"			//******
#include "sfcontrol_funcs.h"		//******控制函数
#include "idle_processing_funcs.h"	//******处理函数
#include "file_service.h"			//文件服务
#include "pkt_tracer.h"				//pkt示踪
#include "session_expect.h"			//******
#include "reload.h"					//再加载
#include "reg_test.h"				//规则检验
#include "memory_stats.h"			//内存统计
#ifdef SIDE_CHANNEL
# include "sidechannel.h"
#endif

#include "dynamic-plugins/sf_dynamic_engine.h"			//动态插件	动态引擎
#include "dynamic-plugins/sf_dynamic_detection.h"		//动态探测
#define PROFILE_PREPROCS_NOREDEF
#include "dynamic-plugins/sf_dynamic_preprocessor.h"	//动态预处理
...
#ifdef TARGET_BASED
# include "target-based/sftarget_reader.h"				//基于目标的/目标阅读器
#endif

#ifdef EXIT_CHECK
# include "cpuclock.h"				//*****cpu锁
#endif
#include "sfActionQueue.h"			//动作队列

#ifdef INTEL_SOFT_CPM
#include "sfutil/intel-soft-cpm.h"	//******
#endif

#include "session_api.h"			//会话相关接口
...
#include "perf_indicators.h"		//性能指示
#endif
 ...
 * Define DLT_OLDPFLOG unless DLT_LANE8023 (Suse 6.3) is already	定义DLT_OLDPFLOG，除非DLT_LANE8023（Suse 6.3）已经存在
 * defined in bpf.h.
 */
...
//下面似乎是各种报警模式	snort的alert-mode模式里面有一种是unsock模式，手册上解释如下：Sets up a UNIX domain socket and sends alert reports to it. External programs/processes can listen in on this socket and receive Snort alert and packet data in real time. This is currently an experimental interface. 
...
#define ALERT_MODE_OPT__CONSOLE    "console"	//控制台
#define ALERT_MODE_OPT__CMG        "cmg"		//*****Computer Measurement Group
...
#ifdef MPLS		//多协议标记交换(Multi-Protocol Label Switching)	https://blog.csdn.net/qq_42868046/article/details/82391807?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522160134850119724848330521%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=160134850119724848330521&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~top_click~default-1-82391807.first_rank_ecpm_v3_pc_rank_v2&utm_term=MPLS&spm=1018.2118.3001.4187
# define MPLS_PAYLOAD_OPT__IPV4      "ipv4"
# define MPLS_PAYLOAD_OPT__IPV6      "ipv6"
# define MPLS_PAYLOAD_OPT__ETHERNET  "ethernet"
#endif

#define DEFAULT_PAF_MAX  16384		//默认******最大
 ...
 */
 ...
 /*我们应该在SnortIdle（）中使用一个较小的数字来进行空闲修剪******，而不是使用16k作为Flowcount，以避免AAB。
 *现在在snort空闲情况下，我们将使用in AAB timeout（Miminum可配置AAB值）来修剪会话。
 *经过测试发现，平均0.2毫秒就可以修剪一个会话。
 *FlowCount=（AAB超时/修剪onesession的时间）/3（tcp+udp+ip）
 */

#define AAB_THRESHOLD 250					//阈值
#define TIME_TO_PRUNE_ONE_SESSION 0.2		//同上
const uint32_t FLOW_COUNT = (AAB_THRESHOLD/TIME_TO_PRUNE_ONE_SESSION) / 3;//流量计数常量
volatile int detection_lib_changed = 0;		//不允许编译器在读取时优化(读取最实时的值)	https://blog.csdn.net/lanhuazui10/article/details/108297646?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522160135000419726892407202%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=160135000419726892407202&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~sobaiduend~default-3-108297646.first_rank_ecpm_v3_pc_rank_v2&utm_term=volatile+int&spm=1018.2118.3001.4187
#ifdef SNORT_RELOAD							//******再加载
extern volatile bool reloadInProgress;		//是否正在从新加载	利用extern关键字，使用在后边或其他文件中定义的变量。	https://blog.csdn.net/xingjiarong/article/details/47656339?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522160135067619726892442794%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=160135067619726892442794&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~first_rank_ecpm_v3~pc_rank_v2-1-47656339.first_rank_ecpm_v3_pc_rank_v2&utm_term=extern+c%E8%AF%AD%E8%A8%80&spm=1018.2118.3001.4187
#endif


/* Data types *****************************************************************/

typedef enum _GetOptArgType			//枚举数据类型 3个枚举常量，按顺序为0,1,2	https://blog.csdn.net/skyflying2012/article/details/22736633?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522160135172519725255539204%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=160135172519725255539204&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~first_rank_ecpm_v3~pc_rank_v2-1-22736633.first_rank_ecpm_v3_pc_rank_v2&utm_term=enum&spm=1018.2118.3001.4187
{
    LONGOPT_ARG_NONE = 0,
    LONGOPT_ARG_REQUIRED,
    LONGOPT_ARG_OPTIONAL

} GetOptArgType;					//一个名为GetOptArgType的，类型为_GetOptArgType的枚举结构 arg-参数

/* Externs *******************************************************************/
/* Undefine the one from sf_dynamic_preprocessor.h */
#ifdef PERF_PROFILING				//性能分析
extern PreprocStats detectPerfStats, decodePerfStats, metaPerfStats,		//PreprocStats 预处理相关
...
/* for getopt 获取选项*/		//以下仅仅是声明，定义已经在外部完成
extern char *optarg;	//参数
extern int optind;
extern int opterr;
extern int optopt;
...
int SnortMain(int argc, char *argv[])
{
    ...
    SnortInit(argc, argv);				//包括InitSingnals()	InitGlobals()		Init_winsock() ParseCmdLine()等各种初始化函数
...
static void ParseCmdLine(int argc, char **argv)			//命令行参数解析
{
    int ch;
    int option_index = -1;
    ...
    */
    ...
    /* Set the run mode based on what we've got from command line 在有命令前设置运行模式*/
    ...
    {
        sc->no_alert = 1;
        sc->run_mode = RUN_MODE__PACKET_LOG;			//模式为log
    }
...
void SnortInit(int argc, char **argv)
{
    ...
    /* chew up the command line 获取命令行命令*/
    ParseCmdLine(argc, argv);

    switch (snort_conf->run_mode)//运行模式选择
    {
